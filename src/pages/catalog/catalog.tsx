import styles from './catalog.module.css';
import type { Endpoint, Entity, Info } from '../../shared/api/types';
import type { SingleValue } from 'react-select';
import { useCallback, useMemo, useState } from 'react';
import { formatDate } from '../../shared/utils/date';
import { Link } from 'react-router';
import { Grid, Item } from '../../widgets/grid/grid';
import { useFetch } from '../../shared/hooks/useFetchCatalog';
import { Loader } from '../../shared/components/loader/loader';
import { internalPaths } from '../../shared/constants/routes';
import { Select, type Option } from '../../shared/components/select/select';
import { api, endpoints } from '../../shared/api/api';
import { ErrorMessage } from '../../shared/components/error-message/error-message';

const sortingOptions: Option[] = [
	{ value: '', label: 'none' },
	{ value: 'create_asc', label: 'create date ASC' },
	{ value: 'create_desc', label: 'create date DESC' },
];

export const Catalog: React.FC<{catalog: Endpoint}> = ({catalog}) => {
	console.log('rerender')

	const { data, loading, error } = useFetch<Info<Entity[]>>(api[catalog].list());
	const [selectedOption, setSelectedOption] = useState<SingleValue<Option>>(null);

	console.log(data)

	const sortedData = useMemo(() => {
		if (!data || !data.results) {
			return null;
		}

		let sorted = [...data.results];

		switch (selectedOption?.value) {
			case 'create_asc':
				sorted = sorted.sort((cur, next) => new Date(cur.created).getTime() - new Date(next.created).getTime());
				break;
				
			case 'create_desc':
				sorted = sorted.sort((cur, next) => new Date(next.created).getTime() - new Date(cur.created).getTime());
				break;

			default:
				break;
		}

		return sorted;
	}, [data, selectedOption?.value]);

	// const handleFetchMore = () => {
	// 	if (hasMore && data && data.info) {
	// 		setSelectedOption({ value: '', label: 'none' })
	// 		url = data?.info?.next;
	// 	}
	// };

	const handleFilterUpdate = useCallback((newVal: SingleValue<Option>) => {
		setSelectedOption(newVal);
	}, []);

	return (
		<>
			<h1 className={styles.title}>{catalog ?? '???'}</h1>
			<div className={styles.select_wrapper}>
				<Select
					value={selectedOption}
					defaultValue={null}
					options={sortingOptions}
					onChange={handleFilterUpdate}
				/>
			</div>
			<PageContent
				category={catalog}
				sortedData={sortedData}
				loading={loading}
				error={error}
			/>
		</>
	);
};

interface PageContentProps {
	category: string | undefined;
	sortedData: Entity[] | null;
	loading: boolean;
	error: null | string;
}

const PageContent: React.FC<PageContentProps> = ({ category, sortedData, loading, error }) => {
	if (error !== null) {
		return (
			<div className={styles.error_container}>
				<ErrorMessage
					message="Ошибка при выполнении запроса"
					description={error}
				/>
			</div>
		);
	}

	if (loading) {
		return (
			<div className={styles.loader_container}>
				<Loader />
			</div>
		);
	}

	if (sortedData === null || sortedData.length === 0) {
		return <p>Data Not Found</p>;
	}

	if (category === undefined || endpoints.indexOf(category) === -1) {
		return <p>Category {category} doesn't exist!</p>;
	}

	return (
		<CategoryContent
			data={sortedData}
			category={category}
		/>
	);
};

interface CategoryContentProps {
	data: Entity[];
	category: string;
}

const CategoryContent = ({ data, category }: CategoryContentProps) => {
	return (
		<Grid className={styles.content}>
			{data?.map(item => {
				return (
					<Link
						key={item.id}
						to={internalPaths.item(category, String(item.id))}
						className={styles.item_wrapper}
					>
						<Item>
							<div className={styles.item_content}>
								<div className={styles.item_header}>
									<h3>{item.name}</h3>
									<p className={styles.item_id}>[{item.id}]</p>
								</div>
								<p className={styles.item_id}>created: [{formatDate(item.created, 'dd.mm.yyyy')}]</p>
							</div>
						</Item>
					</Link>
				);
			})}
		</Grid>
	);
};
